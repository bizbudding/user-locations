...
final class User_Locations_Fields {
	...
	public function load_opening_hours( $value, $post_id, $field ) {
		$value = get_field( 'opening_hours', 'user_4' );
		// echo '<pre>';
	    // print_r($field);
	    // echo '</pre>';
		// $value[] = array(
			// 'field_576be07d6e1f8' => '01:00',
			// 'field_576be21ae95e7' => '01:00',
		// );
		return $field;
		// $array = array(
		// 	'test_1' => 'Test 2',
	...
	public function load_user_fields( $value, $post_id, $field ) {
		trace($field);
		$field_group_key = isset($field['field_576c3d1b190dc']) ? $field['field_576c3d1b190dc'] : '';
	...
	public function load_user_fields_og( $value, $post_id, $field ) {
		...
		// Get current user data and taxonomies
		...
		if ( in_array( $field['name'], $meta_fields ) ) {
			// if ( in_array( $field['name'], $this->get_complex_fields() ) ) {
				// return $meta[$field['name']];
			// }
			// return $meta[$field['name']][0];
			return get_user_meta( $data->ID, $field['name'] );
			// $blah = get_field( $field['name'], 'user_' . $user->ID );
		}
